module.exports = app => {
  class Controller extends app.Controller {
    ...
    async request() {
      ...
      if (index) {
        const customer=this.app.sessions.get(ctx.cookies.get("sessionId",{signed:false,encrypt:false}))
        ...
        const order = await this.ctx.model.Order.findById(
          chat.orderId
        ).exec();
        this.ctx.body = {
          param,
          totalFee: order.totalFee,
          service: `${order.service.category}-${order.service.name}-${
            order.service.type
          }`
        };
      } else {
